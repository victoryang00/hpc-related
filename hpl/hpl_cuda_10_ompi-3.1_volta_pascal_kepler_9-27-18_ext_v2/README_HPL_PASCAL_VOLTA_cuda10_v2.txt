...
These HPL binaries were built with either openmpi  3.1.0 and cuda 10.0 and intel mkl.

Change History :
***************

hpl_cuda_10_ompi-3.1_volta_pascal_kepler_9-27-18_ext - initial release of this package

hpl_cuda_10_ompi-3.1_volta_pascal_kepler_9-27-18_ext_v2 - no change to xhpl binary files. Only adds example run script, HPL.dat file and sample output for DGX2 system with 16xV100 GPUs .......
...
xhpl_cuda-10.0-dyn_mkl-static_ompi-3.1.0_gcc4.8.5_9-26-18    # xhpl executible
xhpl_cuda-10.0-dyn_mkl-dyn_ompi-3.1.0_gcc4.8.5_9-26-18   # same as above with dnamic link to mkl
...
Using NB values greater than 1024 requires setting. This is most likely to be useful in cases with 2 or less GPUs/node and 1:1 topology and when CPUs are very powerful and also when system memory is very large allowing for ver large N values.
...
Adjusting the clock to prevent the GPU from boosting to the max clock will probably help performance and reduce variability a little bit. For example the command line below will adjust the clock for SXM2 P100 to 1392 or 1404MHz typically works pretty well. To find available clock choices for the GPU you are running on you can run "nvidia-smi --query-supported-clocks=gr --format=csv -i 0". It is generally best to find a value as high as possible but just when you begin to hit the power limit on the slowest ranks and clock throttling starts. Tuning this value will tend to be more important when running on large number of nodes and less critical for running on a single node. 
	...
	nvidia-smi -ac 715,1404		# boosted clock for P100......typically works well for P100 SXM2, you want to tune this value to limit clock throttling and maximize performance for your configuration or for different GPUs
        nvidia-smi -ac 877,1230         # base clock for V100 PCIe
...
For running HPL you want tomake sure that your system is generally configured for performance. In some cases there may be system BIOS settings that tradeoff performance vs power saving and or define how the system fans may run trading off system fan noise vs cooling etc. You will want to adjust any settings of that type for best performance mode when running HPL.

It is also good practice to verify linux system settings for CPUs under /sys/devices/system/cpu are set to state for maximum performance. Something similar to below may be useful.
...
For HPL it is also good idea to make sure that "Transparent Huge Pages (THP)" is enabled. One way to do this with command below :
...
When running HPL on multiple nodes it can be useful to take advantage of multiple network adapters if available. You can bind the NICs to GPUs based on topology/affinity of the system. One way to do it when using openmpi for example as below using "OMPI_MCA_btl_openib_if_include" environment variable. The command "nvidia-smi topo -m" can be useful to identify the best mapping for NICs and GPUs.
...
1. You need a minimum of 2 GPUs to run. Running on a single GPU is possible in some cases but when N becomes large enough it will crash.

2. Under some conditions with openmpi 1.10.x based binaries and run env when running on >1 node HPL may generate a FAIL residual check on initial run. To avoid this issue you can simply run a twice and the second test will PASS. It has also been observed that setting "TEST_SYSTEM_PARAMS=0" will sometimess avoid the issue. Lastly you can use the openmpi 3.1.x version of the binary which does not show the issue.
