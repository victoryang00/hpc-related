...
6) inline assembly is a littile hard to control program by C++ variables,
if-esle, for-loop等。 7) HIPCC does not allocate SGPR/VGPR very well r 8) GCN
Limit: s_load_DWORDx4, x8, X16 SGPR must begins x4 address 9) Inline Assembly is
hard to have correct VGPR/SGPR settings:

So: have to use Pure Assembly
Step1:  Build and Dump ISA from Code.
Step2:  Generate .S llvm assembly    :  extractkernel -i ./a.out
Step3:  Generate .co Code Object by  :  /opt/rocm/hcc/bin/clang -x assembler
-target amdgcn--amdhsa -mcpu=gfx900 -mno-code-object-v3 a.s -0 a.co Step4:  Load
a.co from test code
...
/*
    ...
    Eavery 2x Row are packed into 1 Row:  Row0K0, Row1K0, Row0K1, Row1K1,
   Row0K2, Row1K2
   ...
   Row0K2, Row1K2
    ...
    //This version will send out Constant Loading Command Immediately after it
   has be used
...
__device__ void JumpToEnd() { asm volatile("s_branch label_end\n"); }
...
__device__ void InitSGPRAllocations()
{
    ...
    SGPR offset
        ...
        N					9
        K					10
        matrixA_base		12	13		s_load_dwordx4 sgpr_dst[4],
    sgpr_base[2], offset,   number a 20-bit integer constant matrixB_Base
    14	15		global_load_dword vgpr_dst, vgpr_addr[2], sgpr_saddr[2]
    ...
    */
    ...
    asm volatile(".set s_BlockDim_y,                64\n");
}
...
__device__ void InitMatrixABInfo()
{
    ...
    asm volatile(
        "s_load_dwordx2  s[s_matrixA_base:s_matrixA_base+1], s[4:5], 0x8  \n");
    asm volatile(
        "s_load_dwordx2  s[s_matrixB_base:s_matrixB_base+1], s[4:5], 0x10 \n");
    asm volatile(
        "s_load_dword    s[s_M],							  s[4:5], 0x20 \n");
    asm volatile(
        "s_load_dword    s[s_N],							  s[4:5], 0x24 \n");
    asm volatile(
        "s_load_dword    s[s_K],							  s[4:5], 0x28 \n");
    asm volatile(
        "s_load_dword	  s[s_lda],							  s[4:5], 0x2c \n");
    asm volatile(
        "s_load_dword	  s[s_ldb],							  s[4:5], 0x30 \n");
    ...
    asm volatile("s_lshl_b32      s[s_matixA_tile_offset],  s[hipBlockIdx_y], "
                 "6 \n"); // macro tile size 64x256

    // Per-wave + 16x FP32
    // asm volatile("v_readfirstlane_b32	s[s_matixA_tile_offset+1],
    // v[hipThreadIdx_x]\n"); asm volatile("s_lshr_b32
    // s[s_matixA_tile_offset+1],  s[s_matixA_tile_offset+1], 6\n");  //wave_id
    // asm volatile("s_lshl_b32			s[s_matixA_tile_offset+1],
    // s[s_matixA_tile_offset+1], 5\n");  //wave_id * 32 asm volatile("s_add_u32
    // s[s_matixA_tile_offset],    s[s_matixA_tile_offset],
    // s[s_matixA_tile_offset+1]\n");  //m_base per wave
    asm volatile("s_mov_b32				s[s_matixA_tile_offset+1],  0 \n");

    // Computer Matrix B's N offset for 4 waves
    // thread_N_offset = hipThreadIdx
    // asm volatile("v_and_b32		 v[v_matrixB_base],  v[hipThreadIdx_x],
    // 0x3f\n");
    ...
    asm volatile(
        "s_lshl_b32	s[s_matixB_tile_offset], s[hipBlockIdx_x], 8\n"); //*Macro
                                                                      // tile
                                                                      // 64x256
    ...
    asm volatile("s_mul_i32     s[s_matrixA_sgpr_addr],   "
                 "s[s_matixA_tile_offset],  s[s_lda]\n");
    asm volatile("s_mul_hi_u32  s[s_matrixA_sgpr_addr+1], "
                 "s[s_matixA_tile_offset],  s[s_lda]\n");
    asm volatile("s_add_u32		s[s_matrixA_base],    s[s_matrixA_base], "
                 "s[s_matrixA_sgpr_addr] \n");
    asm volatile("s_addc_u32    s[s_matrixA_base+1],  s[s_matrixA_base+1], "
                 "s[s_matrixA_sgpr_addr+1] \n");
    ...
    asm volatile("s_mul_i32     s[s_matrixB_sgpr_addr],   "
                 "s[s_matixB_tile_offset],  s[s_ldb]\n");
    asm volatile("s_mul_hi_u32  s[s_matrixB_sgpr_addr+1], "
                 "s[s_matixB_tile_offset],  s[s_ldb]\n");
    asm volatile("s_add_u32		s[s_matrixB_base],    s[s_matrixB_base],   "
                 "s[s_matrixB_sgpr_addr] \n");
    asm volatile("s_addc_u32    s[s_matrixB_base+1],  s[s_matrixB_base+1], "
                 "s[s_matrixB_sgpr_addr+1] \n");

    asm volatile("v_mul_i32_i24       v[v_matrixB_base],    s[s_ldb], "
                 "v[v_matrixB_base]\n");
    asm volatile("v_mul_hi_i32_i24    v[v_matrixB_base+1],  s[s_ldb], "
                 "v[v_matrixB_base]\n");

    asm volatile(
        "s_mov_b32     s[s_matrixB_sgpr_addr],    s[s_matrixB_base]\n");
    asm volatile(
        "s_mov_b32     s[s_matrixB_sgpr_addr+1],  s[s_matrixB_base+1]\n");
}

__device__ void PreConstLoadMatrixA()
{
    asm volatile(
        "s_mov_b32		   s[s_matrixA_sgpr_addr], 	s[s_matrixA_koffset]\n");
}

__device__ void PostConstLoadMatrixA()
{
    asm volatile(
        "s_add_u32  s[s_matrixA_koffset], s[s_matrixA_koffset], 8*4\n"); // FP32
                                                                         // * K8
                                                                         // =
}
...
{
    asm volatile(
        "s_load_dwordx8   s[s_matrixA_m16:s_matrixA_m16+7], "
        "s[s_matrixA_base:s_matrixA_base+1],     s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8   s[s_matrixA_m16+8 : s_matrixA_m16+15], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8  s[s_matrixA_m16+16 : s_matrixA_m16+23], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8  s[s_matrixA_m16+24 : s_matrixA_m16+31], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8  s[s_matrixA_m16+32 : s_matrixA_m16+39], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8  s[s_matrixA_m16+40 : s_matrixA_m16+47], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8   s[s_matrixA_m16+48 : s_matrixA_m16+55], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");

    asm volatile(
        "s_load_dwordx8  s[s_matrixA_m16+56 : s_matrixA_m16+63], "
        "s[s_matrixA_base:s_matrixA_base+1], s[s_matrixA_sgpr_addr]\n");
    asm volatile("s_add_u32        s[s_matrixA_sgpr_addr], 	"
                 "s[s_matrixA_sgpr_addr], s[s_lda]\n");
}
...
__device__ void GlobalLoadMatrixB(int groupIndex)
{
    if (groupIndex == 0) {
        ...
        asm volatile("global_load_dwordx4 v[v_matrixB_k8A:v_matrixB_k8A+3],  "
                     "v[v_matrixB_base:v_matrixB_base+1],  "
                     "s[s_matrixB_sgpr_addr:s_matrixB_sgpr_addr+1]\n");

        // 2XFP32 == 4DWORDs = 16 bytes
        asm volatile("s_add_u32			  s[s_matrixB_sgpr_addr],   "
                     "s[s_matrixB_sgpr_addr], 16\n");
        asm volatile("s_addc_u32		  s[s_matrixB_sgpr_addr+1], "
                     "s[s_matrixB_sgpr_addr+1], 0\n");
        asm volatile("global_load_dwordx4 v[v_matrixB_k8A+4:v_matrixB_k8A+7],  "
                     "v[v_matrixB_base:v_matrixB_base+1],  "
                     "s[s_matrixB_sgpr_addr:s_matrixB_sgpr_addr+1]\n");
        ...
        asm volatile("global_load_dwordx4 v[v_matrixB_k8B+0:v_matrixB_k8B+3],  "
                     "v[v_matrixB_base:v_matrixB_base+1],  "
                     "s[s_matrixB_sgpr_addr:s_matrixB_sgpr_addr+1]\n");
        ...
        asm volatile("s_addc_u32		  s[s_matrixB_sgpr_addr+1], "
                     "s[s_matrixB_sgpr_addr+1], 0\n");
        asm volatile("global_load_dwordx4 v[v_matrixB_k8B+4:v_matrixB_k8B+7],  "
                     "v[v_matrixB_base:v_matrixB_base+1],  "
                     "s[s_matrixB_sgpr_addr:s_matrixB_sgpr_addr+1]\n");
...
__device__ void DotProductMacros()
{
    asm volatile(".macro outerDotproduct_one_sum sum_offset, v_matB_offset\n \
					...
					s_load_dwordx8  s[s_matrixA_m16+8 *(0x7 & \\sum_offset):s_matrixA_m16+8 *(0x7 & \\sum_offset)+7], s[s_matrixA_base:s_matrixA_base+1],     s[s_matrixA_sgpr_addr]\n  \
					s_add_u32       s[s_matrixA_sgpr_addr], 	s[s_matrixA_sgpr_addr], s[s_lda]\n  \
	.endmacro \n" ::);
...
__device__ void InitMatrixCInfo()
{
    ...
    // load
    asm volatile(
        "s_load_dwordx2   s[s_matrixC_base:s_matrixC_base+1],  s[4:5], 0x18\n");
    asm volatile(
        "s_load_dword     s[s_ldc],							s[4:5], 0x34\n");
    asm volatile("s_load_dwordx2    s[s_alpha:s_beta],					"
                 "s[4:5], 0x38\n");

    // v_matrixC_base = (M_tile_base  + wave_id* per_wave_M_stride=0) * ldc
    asm volatile("s_lshl_b32			s[s_matrixC_m_tile_offset],     "
                 "s[hipBlockIdx_y], 6 \n"); // 64*256 macro tile size
    //	asm volatile("v_readfirstlane_b32	s[s_matrixC_m_tile_offset+1],
    // v[hipThreadIdx_x]\n"); 	asm volatile("s_lshr_b32
    // s[s_matrixC_m_tile_offset+1],   s[s_matrixC_m_tile_offset+1], 6 \n");
    // asm volatile("s_lshl_b32			s[s_matrixC_m_tile_offset+1],
    // s[s_matrixC_m_tile_offset+1], 5 \n");  //*32 	asm volatile("s_add_u32
    // s[s_matrixC_m_tile_offset],     s[s_matrixC_m_tile_offset],
    // s[s_matrixC_m_tile_offset+1]\n");

    // per-vertex-n offset = hipThreadIdx & 0x3f
    // n offset = N_TILE_offset + per-vertex_n
    asm volatile("s_lshl_b32      s[s_matrixC_n_tile_offset],   "
                 "s[hipBlockIdx_x] , 8\n"); // 32 * 256 macro tile size
    // asm volatile("v_and_b32       v[v_matrixC_base],  0x3f,
    // v[hipThreadIdx_x]\n");
    asm volatile("v_mov_b32       v[v_matrixC_base],  v[hipThreadIdx_x]\n");
    asm volatile("v_add_i32       v[v_matrixC_base], "
                 "s[s_matrixC_n_tile_offset], v[v_matrixC_base]\n");
    asm volatile(
        "v_lshlrev_b32   v[v_matrixC_base], 2,  v[v_matrixC_base]\n"); // FP32 *
                                                                       // 4 -->
                                                                       // Bytes;
    ...
    asm volatile("s_mul_i32		s[s_matrixC_addr],		"
                 "s[s_matrixC_m_tile_offset], s[s_ldc]\n");
    asm volatile("s_mul_hi_u32  s[s_matrixC_addr+1] ,	"
                 "s[s_matrixC_m_tile_offset], s[s_ldc]\n");
    asm volatile("s_add_u32     s[s_matrixC_addr],      s[s_matrixC_addr],     "
                 "  s[s_matrixC_base]\n");
    asm volatile("s_addc_u32    s[s_matrixC_addr+1],    s[s_matrixC_addr+1],   "
                 "  s[s_matrixC_base+1]\n");
...
__device__ void GlboalStoreMatrixC()
{
    ...
    // \n"); asm volatile("s_mov_b32     s[s_matrixC_addr+1],
    // s[s_matrixC_base+1] \n");

    asm volatile(".macro m_store_sum_load_grp2  v_sum_offset\n \
    ...
    asm volatile(".macro m_store_sum_load_grp3  v_sum_offset\n \
				s_waitcnt  vmcnt(30) \n \
    ...
    asm volatile(".macro m_store_sum_load_grp4  v_sum_offset\n \
				s_waitcnt  vmcnt(30) \n \
    ...
    asm volatile(".macro m_store_sum_lastgroup  v_sum_offset\n \
				s_waitcnt  vmcnt(30) \n \
...
/*/
__global__ void packed_store(uint32_t inlen, const uint32_t* in, const uint8_t*
mask, uint32_t* out, long long* auxBuf) { int global_x = hipBlockIdx_x *
...
__global__ void sgemm_wk256_m64x256_t64x1_SQC(const float *__restrict__ matA,
                                              const float *__restrict__ matB,
                                              float *__restrict__ matC, int M,
...
test_kernel(hipLaunchParm lp,
    const float* __restrict__  matA, const float* __restrict__ matB, float*
__restrict__ matC, int M, int N, int K, int lda, int ldb, int ldc, float alpha,
float beta, int matrixCTranspose)
{
...
void test()
{
    ...
    int tests[4][5] = {{256, 20736, 256, 256, 16},
    ...
    HIP_ASSERT(hipMemcpy(deviceA, hostA, A_NUM * sizeof(float),
                         hipMemcpyHostToDevice));
    HIP_ASSERT(hipMemcpy(deviceB, hostB, B_NUM * sizeof(float),
                         hipMemcpyHostToDevice));
    ...
    HIP_ASSERT(hipMemcpy(deviceC, hostC, C_NUM * sizeof(float),
                         hipMemcpyHostToDevice));
    ...
    for (int t = 0; t < 4; t++) {
        ...
        for (int m = tests[t][0]; m <= tests[t][1]; m += tests[t][2]) {
            ...
            void **args;
            hipLaunchKernelGGL(sgemm_wk256_m64x256_t64x1_SQC,
                               dim3(blocks_n, blocks_m, 1), dim3(256, 1, 1), 0,
                               0, (float *)deviceA, (float *)deviceB,
                               (float *)deviceC, test_m, test_n, test_k, lda,
                               ldb, ldc, 1.0, 1.0, 0);
            ...
            double total_G_flops =
                double(test_m) * double(test_n) * double(test_k) * 2 +
                3 * double(test_m) * double(test_n);
            total_G_flops = total_G_flops / 1e9;

            float eventsecond = eventMs / 1000;
            float perf_tflops = total_G_flops / eventsecond;

            printf("M, N , K: [%6d, %6d, %6d] , total GFlops: %5.3f,  costs: "
                   "%5.6f seconds performance:%2.3f TFlops/s\n",
                   test_m, test_n, test_k, (float)total_G_flops, eventsecond,
                   perf_tflops);
        }
    }
    HIP_ASSERT(hipMemcpy(hostC, deviceC, sizeof(float) * C_NUM,
