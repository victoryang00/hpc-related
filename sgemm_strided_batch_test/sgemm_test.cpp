...
hipError_t Sgemm(hipFunction_t &kernel, float *A, float *B, float *C, int M,
                 int N, int K, int lda, int ldb, int ldc, float *alpha,
                 float *beta, int isTranspose, int m_tile_size, int n_tile_size)
{
    char argbuf[68];
    size_t argsize = 0;
...
void test(int m, int n, int k, int m_tile_size, int n_tile_size, int printout,
          int iterations = 10, int padding = 16)
{
    ...
    cout << "address " << deviceA << "," << deviceB << "," << deviceC << "\n";
    {
        ...
        Sgemm(sgemmkernel, deviceA, deviceB, deviceC, test_m, test_n, test_k,
              lda, ldb, ldc, &alpha, &beta, 1, m_tile_size, n_tile_size);
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            Sgemm(sgemmkernel, deviceA, deviceB, deviceC, test_m, test_n,
                  test_k, lda, ldb, ldc, &alpha, &beta, 1, m_tile_size,
                  n_tile_size);
        }
        ...
        double total_G_flops =
            double(m) * double(n) * (k)*2 + 3 * double(m) * double(n);
        total_G_flops = total_G_flops / 1e9;

        float eventsecond = eventMs / 1000 / iterations;
        float perf_tflops = total_G_flops / eventsecond / 1000;

        printf("M, N , K: [%6d, %6d, %6d] , total GFlops: %5.3f,  costs: %5.6f "
               "seconds performance:%2.3f TFlops/s\n",
...
int main(int argc, char **argv)
{
    if (argc != 8) {
        ...
        printf("    Usaga1:   m  n  k m_tile_size n_tile_size iterations "
               "print_out\n");
        printf("                                    K must be 16x, "
               "defaultiterations=10\n");
        printf("                                    sgemm_sqc.co with kernel "
               "name sgemm_sqc\n");
