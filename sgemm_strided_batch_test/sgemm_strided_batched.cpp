...
hipError_t sgemm(hipFunction_t &kernel, const float *A, const float *B,
                 const float *C, int M, int N, int K, int lda, int ldb, int ldc,
                 const float *alpha, const float *beta, int isTranspose,
                 int m_tile_size, int n_tile_size)
{
    char argbuf[68];
    size_t argsize = 0;
    *((void **)&argbuf[argsize]) = (void *)0;
    argsize += sizeof(void *); // The first parameter isignored
    *((void **)&argbuf[argsize]) = (void *)A;
    argsize += sizeof(void *);
    *((void **)&argbuf[argsize]) = (void *)B;
    argsize += sizeof(void *);
    *((void **)&argbuf[argsize]) = (void *)C;
    argsize += sizeof(void *);
    *((int *)&argbuf[argsize]) = M;
    argsize += sizeof(int);
    *((int *)&argbuf[argsize]) = N;
    argsize += sizeof(int);
    *((int *)&argbuf[argsize]) = K;
    argsize += sizeof(int);
    *((int *)&argbuf[argsize]) = lda;
    argsize += sizeof(int);
    *((int *)&argbuf[argsize]) = ldb;
    argsize += sizeof(int);
    *((int *)&argbuf[argsize]) = ldc;
    argsize += sizeof(int);
    *((float *)&argbuf[argsize]) = *alpha;
    argsize += sizeof(float);
    *((float *)&argbuf[argsize]) = *beta;
    argsize += sizeof(float);
    *((int *)&argbuf[argsize]) = 0;
    argsize += sizeof(int);
    ...
    return hipModuleLaunchKernel(kernel, gdx, gdy, 1, 256, 1, 1, 0, 0, NULL,
                                 &config[0]);
}
...
void print_strided_batched(const char *name, float *A, int n1, int n2, int n3,
...
static void show_usage(char *argv[])
{
    std::cerr << "Usage: " << argv[0] << " <options>\n"
              ...
              << "\t--stride_a \t\tstride_a \tGEMM_STRIDED_BATCHED argument "
                 "stride_a\n"
              << "\t--stride_b \t\tstride_b \tGEMM_STRIDED_BATCHED argument "
                 "stride_b\n"
              << "\t--stride_c \t\tstride_c \tGEMM_STRIDED_BATCHED argument "
                 "stride_c\n"
              << "\t--batch_count \t\tbatch_count \tGEMM_STRIDED_BATCHED "
                 "argument batch count\n"
...
static int parse_arguments(int argc, char *argv[], int &m, int &n, int &k,
                           int &lda, int &ldb, int &ldc, int &stride_a,
                           int &stride_b, int &stride_c, int &batch_count,
                           float &alpha, float &beta, sgemm_operation &trans_a,
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if ((arg.at(0) == '-') ||
                ...
                if ((arg == "-h") || (arg == "--help")) {
                    ...
                    std::cerr << "error with " << arg << std::endl;
                    std::cerr << "do not recognize option" << std::endl
                              << std::endl;
                    return EXIT_FAILURE;
                }
            }
                ...
                std::cerr << "error with " << arg << std::endl;
                std::cerr << "option must start with - or --" << std::endl
                          << std::endl;
                return EXIT_FAILURE;
            }
        }
...
bool bad_argument(sgemm_operation trans_a, sgemm_operation trans_b, int m,
                  int n, int k, int lda, int ldb, int ldc, int stride_a,
                  int stride_b, int stride_c, int batch_count)
{
    bool argument_error = false;
    if ((trans_a == operation_none) && (lda < m)) {
        argument_error = true;
        std::cerr << "ERROR: bad argument lda = " << lda << " < " << m
                  << std::endl;
    }
    if ((trans_a == operation_transpose) && (lda < k)) {
        argument_error = true;
        std::cerr << "ERROR: bad argument lda = " << lda << " < " << k
                  << std::endl;
    }
    if ((trans_b == operation_none) && (ldb < k)) {
        argument_error = true;
        std::cerr << "ERROR: bad argument ldb = " << ldb << " < " << k
                  << std::endl;
    }
    if ((trans_b == operation_transpose) && (ldb < n)) {
        argument_error = true;
        std::cerr << "ERROR: bad argument ldb = " << ldb << " < " << n
                  << std::endl;
    }
    if (stride_a < 0) {
        argument_error = true;
    ...
    if (ldc < m) {
        argument_error = true;
        std::cerr << "ERROR: bad argument ldc = " << ldc << " < " << m
                  << std::endl;
    }
    if (stride_c < n * ldc) {
        argument_error = true;
...
void initialize_a_b_c(vector<float> &ha, int size_a, vector<float> &hb,
                      int size_b, vector<float> &hc, vector<float> &hc_gold,
                      int size_c)
{
    srand(size_a);
...
int resultencrypt(unsigned char *data, int datalength,
                  unsigned char **encryptdata, int *encryptlength)
{
    BIO *bio = NULL;
    RSA *publicRsa = NULL;
    int retcode = 0;
    ...
    retcode = RSA_public_encrypt(datalength, data, encryptMsg, publicRsa,
                                 RSA_PKCS1_PADDING);
...
void sgemm_strided_batched(sgemm_operation trans_a, sgemm_operation trans_b,
                           int m, int n, int k, const float *alpha,
                           const float *A, int lda, int stride_a,
                           const float *B, int ldb, int stride_b,
                           const float *beta, float *C, int ldc, int stride_c,
                           int batch_count)
{
    ...
    // rocblas_sgemm_strided_batched(handle, transA, transB, m, n, k, alpha, A,
    //                               lda, stride_a, B, ldb, stride_b, beta, C,
    //                               ldc, stride_c, batch_count);

    // pre-process tailoring to the rocblas api
    stride_a = lda * (transA == rocblas_operation_none ? k : m);
    stride_b = ldb * (transB == rocblas_operation_none ? n : k);
    stride_c = ldc * n;
    // cal Sgemm kernel
    for (int i = 0; i < batch_count; i++) {
        sgemm(sgemmkernel, &(A[i * stride_a]), &(B[i * stride_b]),
              &(C[i * stride_c]), m, n, k, lda, ldb, ldc, alpha, beta, 1, 256,
              64);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    // invalid int and float for rocblas_sgemm_strided_batched int and float
    // arguments
    ...
    if (parse_arguments(argc, argv, m, n, k, lda, ldb, ldc, stride_a, stride_b,
                        stride_c, batch_count, alpha, beta, trans_a, trans_b)) {
        show_usage(argv);
        return EXIT_FAILURE;
    }
    ...
    if (bad_argument(trans_a, trans_b, m, n, k, lda, ldb, ldc, stride_a,
    ...
    */
    ...
    CHECK_HIP_ERROR(hipMemcpy(da, ha.data(), sizeof(float) * size_a,
                              hipMemcpyHostToDevice));
    CHECK_HIP_ERROR(hipMemcpy(db, hb.data(), sizeof(float) * size_b,
                              hipMemcpyHostToDevice));
    CHECK_HIP_ERROR(hipMemcpy(dc, hc.data(), sizeof(float) * size_c,
                              hipMemcpyHostToDevice));

    double time = 0.0;
    hipEventRecord(start, 0);

    sgemm_strided_batched(trans_a, trans_b, m, n, k, &alpha, da, lda, stride_a,
                          db, ldb, stride_b, &beta, dc, ldc, stride_c,
                          batch_count);
    ...
    CHECK_HIP_ERROR(hipMemcpy(hc.data(), dc, sizeof(float) * size_c,
                              hipMemcpyDeviceToHost));

    bool result =
        check_result(ha, hb, hc, hc_gold, alpha, beta, m, n, k, batch_count,
                     stride_a, a_stride_1, a_stride_2, stride_b, b_stride_1,
                     b_stride_2, stride_c, ldc, size_c);
    ...
    if (result == 1) {
        ...
        resultencrypt(source, time_output.length(), &encryptMsg,
                      &encryptlength);
